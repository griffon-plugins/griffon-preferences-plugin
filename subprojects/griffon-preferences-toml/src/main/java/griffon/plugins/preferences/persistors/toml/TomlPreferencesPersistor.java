 ...
 */
public class TomlPreferencesPersistor extends AbstractMapBasedPreferencesPersistor {
    @Inject
    public TomlPreferencesPersistor(@Nonnull GriffonApplication application,
    ...
    protected Map<String, Object> doRead(@Nonnull InputStream inputStream) throws IOException {
        if (inputStream.available() > 0) {
            return new Toml().read(inputStream).toMap();
        }
    ...
    protected void write(@Nonnull Map<String, Object> map, @Nonnull OutputStream outputStream) throws IOException {
        new TomlWriter().write(map, outputStream);
    }
}
